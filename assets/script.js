...
/* ──────────────────────────────────────────────────────────
   SHOPIFY QUICK ADD (forms rendered from snippets)
   ────────────────────────────────────────────────────────── */
function initShopifyQuickAdd() {
  document.querySelectorAll('.shopify-buy-form').forEach(form => {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.description || 'Das Produkt konnte nicht hinzugefügt werden.');
        }

        const card = form.closest('.product-card, .deal-card');
        const productName = card?.querySelector('.product-name, h3')?.textContent || 'Produkt';
        ...
        showToast(`🛒 "${productName}" wurde zum Warenkorb hinzugefügt!`);
      } catch (err) {
        showToast(err.message || 'Fehler beim Hinzufügen zum Warenkorb.');
      } finally {
        if (submitBtn) {
